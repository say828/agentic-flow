...
Claude Code에서는 `agentic-flow`를 marketplace로 추가한 뒤 필요한 플러그인을 설치합니다.
...
Codex에서는 installer가 skill과 ADL runtime을 설치합니다.
...
- Claude용 marketplace 플러그인 설치 대상: `ship`, `spec-orchestrator`, `autonomous-decision-loop`, `agentic-dev`, `sdd`
...
| **agentic-dev** | GitHub Project/Issue, SDD, repo contract, Discord webhook, proof/repair loop를 묶는 통합 개발 workflow |
...
```bash
~/.codex/skills/agentic-dev/scripts/init_agentic_dev.sh "feature name" /path/to/repo --github --project-title "Agentic Dev"
```
...
GitHub Project/Issue 등록, SDD scaffold, repo-local command contract, Discord webhook 예시, proof/repair loop를 하나로 묶은 통합 개발 workflow입니다.
