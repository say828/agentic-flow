...
MARKETPLACE_NAME = "say828-autonomous-decision-loop"
PLUGIN_NAME = "autonomous-decision-loop"
REPO_SLUG = "say828/autonomous-decision-loop"
...
def install_codex(repo_dir: Path) -> None:
    ...
    notify_command = ["python3", str(repo_dir / "runtime" / "codex_notify.py")]
    update_codex_config(config_path, notify_command, previous_notify_path)

    bin_dir = Path.home() / ".local" / "bin"
    ensure_dir(bin_dir)
    wrapper_target = bin_dir / "codex"
    real_codex = resolve_real_codex({repo_dir / "bin" / "codex", wrapper_target}) or "/usr/local/bin/codex"
    env_file = repo_dir / ".env.install"
    env_file.write_text(f"ADL_REAL_CODEX_BIN={real_codex}\n")
    for name in ("codex", "codex-inline-tmux.sh"):
        target = bin_dir / name
        source = repo_dir / "bin" / name
        if target.exists() or target.is_symlink():
            if name == "codex" and not target.is_symlink():
                backup = bin_dir / "codex.adl-backup"
